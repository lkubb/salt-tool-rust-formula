...
We will see a quick setup to configure the ``tool_rust`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_rust`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_rust`` formulas under ``/srv/salt/tool_rust/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_rust/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_rust/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_rust/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_rust/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_rust`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_rust`` formulas under ``/srv/salt/tool_rust/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_rust/parameters/roles
...
We create a configuration for the role ``tool_rust/server`` in ``/srv/salt/tool_rust/parameters/roles/tool_rust/server.yaml``:
...
We create another configuration for the role ``tool_rust/client`` in ``/srv/salt/tool_rust/parameters/roles/tool_rust/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_rust/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_rust/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_rust/parameters/dns:domain/  '/srv/salt/tool_rust/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-rust-as-json:
      test.nop:
        - name: {{ rust | json }}
...
.. code-block:: sls
    ...
    rust-config-file-file-managed:
      file.managed:
        - name: {{ rust.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='rust-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ rust.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``rust`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ rust['winner'] }}
